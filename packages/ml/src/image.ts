/**
 * Pure image helpers for the ML pipeline — no platform dependencies.
 * Decoding (file/uri -> RgbImage) lives in each runtime adapter
 * (packages/ml/src/runtime/node.ts, react-native.ts); everything downstream
 * (resize, letterbox, blob, crop, warp) is shared so desktop and mobile
 * produce byte-identical inputs to the ONNX models.
 */
