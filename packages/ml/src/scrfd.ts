 ...
 *
 * Runtime-agnostic: the ONNX session + Tensor come from the injected OrtBackend
 * (onnxruntime-node on desktop, onnxruntime-react-native on mobile).
 */
export class ScrfdDetector {
  ...
  static async create(config: ModelConfig, backend: OrtBackend, source: ModelSource): Promise<ScrfdDetector> {
    const d = new ScrfdDetector(config)
    d.session = await backend.createSession(source, { executionProviders: ['cpu'] })
    ...
    const probe = new Tensor('float32', new Float32Array(3 * probeSize * probeSize), [1, 3, probeSize, probeSize])
  ...
  async detect(img: RgbImage, detSize: number, detThresh: number, detScale: number): Promise<ScrfDetResult> {
    ...
    const feeds = { [this.inputName]: new Tensor('float32', blob, [1, 3, detSize, detSize]) }
