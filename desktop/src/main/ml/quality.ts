...
export class QualityScorer {
  ...
  static async create(modelPath: string, size = 112): Promise<QualityScorer> {
    const q = new QualityScorer(modelPath, size)
    q.session = await ort.InferenceSession.create(modelPath, {
  ...
  async scoreAligned(aimg: RgbImage): Promise<number> {
    ...
    const feeds = { [this.inputName]: new ort.Tensor('float32', blob, [1, 3, this.size, this.size]) }
