...
async function main() {
  ...
  console.log(`\n=== IDENTITY-LEVEL: 0.45 → 0.60 sweep ===`)
  const configs = [
    { name: 'global 0.45 (current)', threshold: 0.45, mode: 'global' },
    ...
    { name: 'QA 0.55/lowJoin0.60', threshold: 0.55, mode: 'qa', lowJoinSim: 0.60 },
    { name: 'QA 0.60/lowJoin0.60', threshold: 0.60, mode: 'qa', lowJoinSim: 0.60 },
  ]
  const results = []
  ...
  for (const t of [0.45, 0.50, 0.55, 0.60]) {
